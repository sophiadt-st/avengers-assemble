...
export async function loader({ params }) {
    ...
    for (const item of series) {
        promises.push(marvelApi.characters(id).comics.get({ series: item.id, limit: 5 }))
    }
            ...
            {data.map((series) => {
                return (
                    <div key={series.id} className="mb-8">
                        <h2 className="mb-4">{series.name}</h2>
                        <div className="flex gap-3 overflow-x-auto h-96">
                            {series.comics.map((comic) => {
                                return (
                                    <div key={comic.id} className="max-w-sm bg-white rounded-lg h-full w-64">
                                        <div className="h-full w-64">
                                            <img className="rounded-t-lg max-w-full max-h-full object-contain" 
